...
def element_wait(
    ...
    condition_type : str
        The type of condition to be applied. If no valid condition is given, the element waits until the value in
    ...
    webdriver.firefox.webelement.FirefoxWebElement
        The element which was found, if no element was found, either None or an empty list is returned, depending on the
        condition chosen.

    Raises
    ------
    selenium.common.exceptions.TimeoutException
        If no element is found in the given `time`, then a TimeoutException is raised.
    ...
    try:
        element = WebDriverWait(element, time).until(condition((by, val)))
    except TimeoutException:
        msg = f"The value {val} which should be searched in {element} " \
              f"could not be located in {time} " \
              f"seconds given the by strategy {by} "\
